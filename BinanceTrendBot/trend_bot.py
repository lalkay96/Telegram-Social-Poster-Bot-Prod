...
async def send_report():
    ...
    if trending_markets:
        ...
        # MODIFIED: Added enumerate to number the list
...
def run_scanner_job():
    ...
    print("\nScheduler triggered. Running the report job...")
    try:
        asyncio.run(send_report())
    except Exception as e:
        print(f"An error occurred during the scheduled job run: {e}")

# --- Main execution block using the 'schedule' library ---
if __name__ == "__main__":
    print("Bot starting up...")
    
    run_scanner_job()

    schedule.every().day.at("08:00").do(run_scanner_job)
    schedule.every().day.at("14:00").do(run_scanner_job)
    schedule.every().day.at("23:00").do(run_scanner_job)

    print("\nScheduler has been set up. Bot is now in running mode. ⏰")
    print(f"Next scheduled run is at: {schedule.next_run}")
