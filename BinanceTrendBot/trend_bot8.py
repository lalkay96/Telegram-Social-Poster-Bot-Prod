...
def run_scanner_job(loop):
    ...
    try:
        # Run the async function on the provided, persistent loop
...
if __name__ == "__main__":
    ...
    # Get the event loop ONCE at the start.
    loop = asyncio.get_event_loop()

    # Run once immediately, passing the loop
    run_scanner_job(loop)

    # Scheduler: Pass the loop to the job using .do()
    schedule.every().day.at("09:05", YOUR_TIMEZONE).do(run_scanner_job, loop=loop)
    schedule.every().day.at("14:07", YOUR_TIMEZONE).do(run_scanner_job, loop=loop)
    schedule.every().day.at("00:07", YOUR_TIMEZONE).do(run_scanner_job, loop=loop)
